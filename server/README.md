...
- 如果目标节点启用了 `Defaults requiretty` 或执行 `sudo` 时需要交互式密码，请在配置文件的 `hosts` 条目中新增 `"sudo_password"` 字段（未提供时默认复用登录密码），或预先为该用户配置免密 sudo；脚本会自动申请伪终端避免 `no tty present` 报错。
- 后台“新增加速节点”表单同样提供独立的 “sudo 密码” 字段，便于在 SSH 登录密码与 sudo 密码不同的情况下存储；若留空则沿用 SSH 密码或假设已配置免密 sudo。
- 每个节点需要 sudo 权限以安装软件包、写入 `/etc/nginx/conf.d/vmp_edge.conf` 并开放防火墙端口。
- 请确保节点能解析并访问对应发行版的软件源（CentOS 7 默认访问 `mirrorlist.centos.org`）；新版部署流程在检测到镜像不可达时会自动切换到 `vault.centos.org/<版本号>/...` 并追加官方 `nginx.org` 仓库后重试，但在完全离线场景仍建议预先配置企业内 YUM 镜像或预装 rpm 包，以避免首次连接耗时或再次失败。
